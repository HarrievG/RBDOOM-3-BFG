...
intptr_t idInteractable::Invoke(const char *functionName, void *param1) {
	int functionNameHash = idStr::Hash(functionName);
	if(functionNameHash == 241303) { // CanBeInteractedWith
		CanBeInteractedWith();
		return 0;
	};
	return __super::Invoke(functionName, param1);
...
bool idInteractable::HasNativeFunction(const char *functionName) {
	int functionNameHash = idStr::Hash(functionName);
	if(functionNameHash == 241303) { // CanBeInteractedWith
		return true;
	};
...
intptr_t idPlayer::Invoke(const char *functionName, void *param1) {
	...
	if(functionNameHash == 101286) { // Interact
		Interact();
		return 0;
	};
...
intptr_t rcPilotable::Invoke(const char *functionName, void *param1) {
	int functionNameHash = idStr::Hash(functionName);
	if(functionNameHash == 63102) { // Spawn
		Spawn();
		return 0;
...
bool rcPilotable::HasNativeFunction(const char *functionName) {
	int functionNameHash = idStr::Hash(functionName);
	if(functionNameHash == 63102) { // Spawn
		return true;
	};
